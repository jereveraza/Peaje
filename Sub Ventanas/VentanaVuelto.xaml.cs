...
namespace ModuloPantallaTeclado.Sub_Ventanas
{
    ...
    /// </summary>
    public partial class VentanaVuelto : Window, ISubVentana
    {
        ...
        private const byte _maximoCaracteresVuelto = 4;
        #endregion

        #region Constructor de la clase
        public VentanaVuelto(IPantalla padre)
        {
            InitializeComponent();
            _pantalla = padre;
        }
        ...
        private void Grid_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            if (_pantalla.ParametroAuxiliar != string.Empty)
            {
                ...
                {
                    lblMonto.Text = "Ingrese Monto Recibido:";
                    txtMonto.Style = Estilo.FindResource<Style>(ResourceList.TextBoxStyleHighlighted);
                }));
        ...
        {
            FrameworkElement control = (FrameworkElement)borderVentanaVuelto.Child;
            borderVentanaVuelto.Child = null;
            Close();
            return control;
        }
        #endregion
        ...
        public bool RecibirDatosLogica(ComandoLogica comandoJson)
        {
            ...
            try
            {
                ...
                {
                    decimal vuelto = Utiles.ClassUtiles.ExtraerObjetoJson<decimal>(comandoJson.Operacion);
            ...
            else
            {
                if (Teclado.IsConfirmationKey(tecla))
                {
                    ...
                    if (txtMonto.Text.Length >= 0)
                    {
                        ...
                        Utiles.ClassUtiles.InsertarDatoVia( Monto, ref listaDV );

                        EnviarDatosALogica( enmStatus.Ok, enmAccion.T_VUELTO, JsonConvert.SerializeObject( listaDV, jsonSerializerSettings ) );
                ...
                {                 
                    _pantalla.MensajeDescripcion(string.Empty);
                    _pantalla.CargarSubVentana(enmSubVentana.Principal);
                    EnviarDatosALogica(enmStatus.Abortada, enmAccion.T_VUELTO, string.Empty);
                    _pantalla.TecladoOculto();
                }
                ...
                else
                {
                    ...
                    {
                        if (txtMonto.Text.Length < _maximoCaracteresVuelto)
                        {
                            txtMonto.Text += Teclado.GetKeyAlphaNumericValue(tecla);
                        }
                    }
                }
            }
        ...
        {
            _ventanaSimbolo.Close();
            if (txtMonto.Text.Length < 4)
            {
                string sNuevoSimb = item == null ? string.Empty : item?.Descripcion;
                txtMonto.Text = txtMonto.Text + sNuevoSimb;
            }
        }
