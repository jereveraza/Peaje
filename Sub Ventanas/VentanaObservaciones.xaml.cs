...
{
    /// <summary>
    /// Lógica de interacción para VentanaObservaciones.xaml
    /// </summary>
    public partial class VentanaObservaciones : Window, ISubVentana
    {
        ...
        public VentanaObservaciones(IPantalla padre)
        {
            InitializeComponent();
            _pantalla = padre;
        ...
        private void Grid_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            if (_pantalla.ParametroAuxiliar != string.Empty)
            {
                try
                {
                    _listaOpciones = Utiles.ClassUtiles.ExtraerObjetoJson<ListadoOpciones>(_pantalla.ParametroAuxiliar);
                    ...
                    int nroOpcion = 1;
                    foreach (var obs in _listaOpciones.ListaOpciones)
                    {
                        if(_cantidadOpcionesMenu < 10)
                            listBoxMenu.Items.Add(nroOpcion.ToString() + " :\t" + obs.Descripcion);
                        else
                            listBoxMenu.Items.Add(nroOpcion.ToString("00") + " :\t" + obs.Descripcion);
                        nroOpcion++;
                    }
                    _OpcionElegida = 1;
                    ...
                    lblTituloObservaciones.Dispatcher.BeginInvoke((Action)(() => 
                        lblTituloObservaciones.Text = _causa.Descripcion));

                }
                catch (JsonException jsonEx)
                {
                    _logger.Debug("VentanaObservaciones:Grid_Loaded() JsonException: {0}", jsonEx.Message.ToString());
                    _logger.Warn("Error al intentar deserializar una Respuesta de logica.");
                }
                catch (Exception ex)
                {
                    _logger.Debug("VentanaObservaciones:Grid_Loaded() Exception: {0}", ex.Message.ToString());
                    _logger.Warn("Error al intentar deserializar una Respuesta de logica.");
                }
            }
        }
        ...
        {
            FrameworkElement control = (FrameworkElement)borderMenObservaciones.Child;
            borderMenObservaciones.Child = null;
            Close();
            return control;
        }
        #endregion
        ...
        public void ProcesarTecla(Key tecla)
        {
            ...
            if (_enmMenuPrincipal == enmMenuPrincipal.Seleccion)
            {
                ...
                {
                    Utiles.ClassUtiles.InsertarDatoVia(_listaOpciones.ListaOpciones[_OpcionElegida - 1], ref listaDV);
                    Utiles.ClassUtiles.InsertarDatoVia(_causa, ref listaDV);

                    EnviarDatosALogica(enmStatus.Ok, enmAccion.OBSERVACION, Newtonsoft.Json.JsonConvert.SerializeObject(listaDV, jsonSerializerSettings));
                    _pantalla.MensajeDescripcion(string.Empty);
                    _pantalla.CargarSubVentana(enmSubVentana.Principal);
                }
            }
