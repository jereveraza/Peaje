...
namespace ModuloBaseDatos
{
    ...
    /// </summary>
    /// ****************************************************************************************************
    public class SQLExpress : IBaseDatos
    {
        private static NLog.Logger _logger = NLog.LogManager.GetLogger("logfile");
        private int _commandTimeoutLocal = 1, _commandTimeoutEst = 2;
        private string _connectionString, _connectionStringEst, _tbVars;
        private int _via, _estacion;
        private static bool _existeVars = false, _existeConfig = false, _bInit = false;
        ...
        /// ************************************************************************************************
        public SQLExpress(ConfiguracionBaseDatos config)
        {
        ...
        public void Init(ConfiguracionBaseDatos config)
        {
            if (!_bInit)
            {
                ...
                _logger.Info("Inicialización completa");
            }
        }
            ...
            else
            {
                ...
                try
                {
                    ...
                    using (SqlCommand command = new SqlCommand(sConsulta, connection))
                    {
                        ...
                        command.CommandTimeout = _commandTimeoutLocal;
                        ...
                        if (parametros.Accion == eAccionBD.Consulta)
                        {
                            ...
                            if (parametros.Tabla == eTablaBD.Vars && !_existeVars)
                            {
                                ...
                                {
                                    _logger.Error($"Excepcion U/I/D Al crear _tbVars : [{e.Number}:{e.Message}]");
                                }
                            }

                            if (parametros.Tabla == eTablaBD.Vars)
                                result.CodError = EnmErrorBaseDatos.SinResultado; //no consultar vars, no se está usando
                            else
                            {
                                using (SqlDataReader reader = await command.ExecuteReaderAsync())
                                {
                                    if (reader != null)
                                    {
                                        ...
                                        {
                                            result.RespuestaDB += reader[0].ToString();
                                        }
                        ...
                        else if (parametros.Accion == eAccionBD.Update || parametros.Accion == eAccionBD.Insert || parametros.Accion == eAccionBD.Delete)
                        {
                            ...
                            {
                                _logger.Error("Excepcion U/I/D.. [{0}:{1}]", e.Number, e.Message);
                    ...
                    result.DescError = Utility.ObtenerDescripcionEnum(result.CodError);
                }
                catch (SqlException e)
                {
                ...
                {
                    _logger.Error($"InvalidOperation. Consulta Realizada: [{e.Message}]");
                    //Busco el error
            ...
            else
            {
                ...
                if (parametros.Accion == eAccionBD.Procedure)
                {
                    ...
                    switch (parametros.Tabla)
                    {
                        ...
                        case eTablaBD.GetTicketClearing:
                            {
                                sProcedure = "ViaNet.usp_GetTicketClearing";
                                break;
                            }
                ...
                try
                {
                    ...
                    using (SqlCommand command = new SqlCommand(sConsulta, connection))
                    {
                        ...
                        //Establezco la conexión...
                        await connection.OpenAsync();
                        command.CommandTimeout = _commandTimeoutEst;

                        //Leo los datos que me responde la BD al ejecutar el SP
                        if (parametros.Accion == eAccionBD.Procedure)
                        {
                            using (SqlDataReader reader = await command.ExecuteReaderAsync())
                            {
                                if (reader != null)
                                {
                                    ...
                                    if (result.RespuestaDB == "" || result.RespuestaDB == "[]")
                                    {
                                        //Error: No se encontro la busqueda
                                        result.CodError = EnmErrorBaseDatos.SinResultado;
                                    }
                ...
                {
                    _logger.Error($"Otras Excepciones. {sConsulta} [{e.Message}]");
                    //Error: otros errores
                    result.CodError = (EnmErrorBaseDatos.ErrorBaseDatos);
                    result.DescError = Utility.ObtenerDescripcionEnum(EnmErrorBaseDatos.ErrorBaseDatos);
                    return result;
        ...
        public string ProcesarConsulta(SolicitudBaseDatos consulta)
        {
            ...
            else
            {
                ...
                else if (consulta.Expresion == eExpresionBD.Distinct)
                    sRetConsulta += $"SELECT DISTINCT {consulta.Campos} FROM {sTabla}";
            ...
            sRetConsulta += " FOR JSON PATH, INCLUDE_NULL_VALUES";
        ...
        /// </summary>
        /// <param name="eNumer">Numero de la excepción producida</param>
        /// <param name="sError">Descripción del error</param>
                ...
                try
                {
                    ...
                    if (sOpcion == "BUSCAR")
                    {
                        ...
                        if (bRet)
                        {
                            ...
                            _logger.Debug("Obtengo Via/Est/UID almacenado en local [{0}/{1}/{2}]",
                                                Configuraciones.Instance.Configuracion.Via,
                                                Configuraciones.Instance.Configuracion.Estacion,
                                                Configuraciones.Instance.Configuracion.UID);
                        }
                    }
        ...
        public void ClearPools()
        {
            ...
            SqlConnection.ClearPool(new SqlConnection(_connectionStringEst));
        }
    }
}